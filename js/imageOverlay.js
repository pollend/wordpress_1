...
function ImageOverlay( gallerID,  itemID){
	...
	if(lockImageProgression === false)
	{
		...
		if(jQuery("#image-overlay-image").attr("src") !== lItem[0])
		{
			...
			{
				jQuery("#image-overlay-left").css({display:"none"});
				jQuery("#image-overlay-right").css({display:"none"});
			}
			else
			{
				jQuery("#image-overlay-left").css({display:"block"});
				jQuery("#image-overlay-right").css({display:"block"});
			}
